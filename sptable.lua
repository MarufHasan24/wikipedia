...
function pp.header(t,Args,p_sub,pos_label,group_col,VTE_text,full_table,results_header_txt)
	...
	local played_head_text = Args['pld_header'] or '<abbr title="খেলা হয়েছে">খেলা</abbr>'
	local draw_head_text = Args['draw_header'] 
		or (yesno(Args['use_tie'] or 'no') and '<abbr title="টাই হয়েছে">টাই</abbr>') or '<abbr title="ড্র হয়েছে">ড্র</abbr>'
	local group_head_text = Args['group_header'] or '<abbr title="গ্রুপ">গ্রুপ</abbr>'
	local team_head_text = Args['team_header'] or 'টিম'
	local away_goals_head_text = Args['away_goals_header'] or '<abbr title="দূর থেকে পাওয়া গোল">দূ.গো</abbr>'
	local bonus_head_text = Args['bonus_header'] or '<abbr title="বোনাস পয়েন্ট">বো.প</abbr>'
	local rw_head_text = Args['rw_header'] or '<abbr title="যতগুলো রাউন্ড জিতেছে">রা.জ</abbr>'
	...
	if pts_perc_first then
		if show_points then
			tt = p_sub.colhead(tt,28,'<abbr title="পয়েন্টসমূহ">পয়েন্ট</abbr>')					-- Points col
		end
		if show_perc then
			tt = p_sub.colhead(tt,36,'<abbr title="শতকরা জয়">শ.জ</abbr>')			-- Win percentage col
		end
		if show_ppg then
			tt = p_sub.colhead(tt,36,'<abbr title="প্রতি গেমে পাওয়া পয়েন্ট">প.গ.প</abbr>')		-- Points per game col
		end
	end
	if show_played then
	...
	if full_table then
		if show_win then
			tt = p_sub.colhead(tt,28,'<abbr title="জয়">জ</abbr>')			-- Win col
		end
		if loss_first then
			if show_loss then
				tt = p_sub.colhead(tt,28,'<abbr title="হার">হ</abbr>')		-- Loss col
			end
			if show_draw then
	...
	if not pts_perc_first then
		if show_points then
			tt = p_sub.colhead(tt,28,'<abbr title="পয়েন্টসমুহ">পয়েন্ট</abbr>')					-- Points col
		end
		if show_perc then
	...
	if tonumber(digits) and tonumber(num) then
		...
		if digits == 1 then
			if n >= 0 and n < 10 then
				return '<span style="visibility:hidden;color:transparent;">০</span>' .. num
			end
		elseif digits == 2 then
			if n >= 0 and n < 10 then
				return '<span style="visibility:hidden;color:transparent;">০০</span>' .. num
...
function pp.row(frame,t,Args,p_sub,notes_exist,hth_id_list,full_table,rand_val,team_list,team_code_ii,ii_start,ii_end,ii_fw,bg_col,N_teams,ii,ii_show)
	...
	if (tonumber(Args['adjust_points_'..team_code_ii]) and tonumber(Args['startpoints_'..team_code_ii])) then
		table.insert(t,'[[Category:বিন্যাস এবং স্টার্টপয়েন্ট উভয় সম্বলিত স্পোর্টস টেবিল ব্যবহার করে এমন পাতা]]\n')
	end
	if pts_perc_first then
		if rank_points then
