  ...
  if (
    ...
    try {
      if (skin === "minerva") {
        ...
        document.getElementById("p-cactions-checkbox").onmouseover = finalTouch;
      } else {
        finalTouch();
      }
    } catch (e) {
    ...
    if (typeof strObj.problems !== "string") {
      ...
      if (tags && pagename && creator && creation && sender) {
        ...
        if (typeof strObj.problems === "object") {
          if (!multiple) {
            strObj.string = `<nowiki>{{subst:টেমপ্লেট:বার্তা প্রদান|page=${pagename}|problem=${strObj.problemDes}${strObj.suggesition}|creator=${creator}|sender=${sender}|days=${creationDate.days}|date=${creationDate.date}|sign=~~~~}}</nowiki>`;
          } else if (multiple === true && tags.length > 1) {
            strObj.string = `<nowiki>{{subst:টেমপ্লেট:বার্তা প্রদান-একাধিক|page=${pagename}|problem=${strObj.problemDes}${strObj.suggesition}|creator=${creator}|sender=${sender}|days=${creationDate.days}|date=${creationDate.date}|sign=~~~~}}</nowiki>`;
          }
